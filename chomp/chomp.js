/* ==========================================================================
   /chomp — wrapper behaviour. Two small things, both about the keyboard.

   The game runs in the iframe and owns its own Escape ladder (pause menu -> ...). Key events do not cross a frame boundary, so once the
...
const frame = document.querySelector('.chomp-frame');

// Forward the wrapper's query string into the game — ?seed=123 reproduces a
// world and ?debug=1 opens the dev tooling, and both are typed against /chomp,
// not against the internal tree path. Done before anything else so the first
// load is the right one (reassigning src just replaces the in-flight request).
if (window.location.search) {
  frame.src = '/games/chomp/index.html' + window.location.search;
}
