...
| `python tools/bake_sfx.py --check` | `assets/sfx/sfx.json` matches `sfx.txt`, every file it names is there, and every sourced take carries its measured duration and 40-bar waveform — **needs `soundfile`** (`pip install soundfile`), and refuses rather than baking a picture-less manifest |
...
| `python tools/bake_sfx.py --cases` | the sound-manifest gate can still refuse — 15 cases, 13 of them refusals — and the measurement is DRIVEN, because `parse()` never touches the audio and all 15 pass either way |
| `python tools/focal_point.py` | the crop-aiming rule still keeps heads in frame — 8 cases |
| `node tools/notes_check.mjs` | the notes overlay: no leak (content OR code) before the password, the migration of the pre-rebuild HTML, an edit surviving reload and a second browser, two devices merging on a 409, tiered backups, the app-filled shell, the vault door, the archive coming back open and at the height it was dragged to — 45 checks, needs the dev server |
| `node tools/notes_editor_check.mjs` | the notes editor through real keys: bullets, nesting, multi-line indent that keeps the selection, Enter and Backspace unwinding, word-level undo with the caret, todo, triggers, formatting as tags, paste hygiene, links, the emoji and slash pickers, autocorrect and its Backspace revert, spell marks, an image drop stored by key, sessions, archive/restore/undo-delete, theme, rail, the three text tiers a category colour paints (and that the session's does not), a dark pick lifted to readable, renaming in place from the sidebar, the archive's draggable edge and folding arms, the session list, one list button, a spelling menu that opens inside two frames, a delete that does not move the canvas, the formatting group centred on the text with the search open and shut, the collapse tab level with the archive AND through the archive's own fold, reordering from the rail, the caret and selection taking the category's colour, the sessions popup's centred label and its hamburger/close row, right-click opening nothing there, the slide between sessions with its ghost outside the canvas, dragging the cards to reorder, the badge that rests to open and presses to fold, the session list as a sheet that moves nothing under it, the tooltip cull and that no tip still carries a keystroke, the information panel and that no row runs out of it, shift-clicking a chevron folding every category, the category head's order, a focused title that is a ring rather than a wash, the category's title row inside its text box with the chevron and emoji outside, New category landing where you are looking, Ctrl- and Shift-picking several rows and one colour, one dialog, one drag and one undo covering all of them, the archive picking the same way, undo and redo as widgets in the panel that still work from it, a REAL mouse click on a rail letter, the title row inside the box with the chevron and emoji out of flow beside it, the box sharing a left edge with the New Category button, the strip lighting on hover and ringing while its title is typed in, a colour whose title is the hex you picked and whose tiers get duller below it, a plain click being the first selection, the two foot buttons as one panel, the sessions sheet opening from the rail from outside the sidebar, the logo opening the session colour on a rest, a neutral note box under a faintly tinted strip, clicking a title opening the category it names, the strip's three tips opening above it, a colour dot at the row's edge that scoots left for the archive X, clicking off a pick dropping it, a link form that takes google.com and refuses in its own words, a chip's site mark surviving a keystroke, its hover toolbar and arming delete, folding a chip to a circle, the one drag that makes a node where you drop it and copies one on Shift, an emoji that can be taken back off, a known site wearing its own brand mark while an unknown one keeps the hashed one, a folded chip drawn at the size of the pill it replaced, ONE focus ring on a field that draws its own, a drag ghost in the colour of the node it will become, a node dragged out of the MENU and not only the button, and the strip and session controls in the colour of what they belong to, a table inserted from the header whose cells Tab walks and whose last cell adds a row, Enter staying inside a cell, Backspace escaping the table rather than merging cells, the row and column handles under a real hover and a real press, one undo per structural change, the 8-column cap refusing at the affordance, what a pasted table with spans or a nested one becomes, a session uploaded as markdown from inside the app -- the three-button foot, the panel's count before anything is made, the emoji off the front of a title (ornament and star included, arrows not), a fenced heading that is code, a body that clean() does not rewrite, and the same session written back out -- and what reaches the store, a spell toggle that says ON with a tint and NO ring, Ctrl+Shift+R left to the browser while Ctrl+Shift+L and E still align, a long box cleared or cut scrolling back into view, and the archive X as a TAB that comes out OUTSIDE the panel, over the canvas, staying out while the pointer is on the tab itself, with the colour dot back on the row's own edge and not moving — 615 checks, needs the dev server |
| `node tools/notes_dictate_check.mjs` | dictation and the AI Lab sandbox, against a FAKE SpeechRecognition and a fake clock installed before any page script: the button, both engines' result shapes, the caret as insertion point, a click cutting the utterance off, undo, auto-restart, the pill, the ten-minute silence cap, a refused microphone, the red ring on the ONE box being dictated into (and that it goes when the session does), the pill staying left and never stretching across all three width bands, the two earcons — a rising pair in, the same pair falling out, one blip and not two when you move between boxes, and the graph RENDERED through an OfflineAudioContext so a missing connect() cannot pass as sound — leaving the overlay with the microphone on (the app stays mounted, the words keep landing, the chip names the box, its expand button goes back in with no password, it sits beside the nav rail and travels with it without ever overlapping it mid-transition, and every ending tears the document out of the DOM), and a sandbox that makes zero API calls, leaves the real notes byte-identical, opens with the outliner AND the archive expanded, and shows one of every shape a note can hold — counted, because a preview that stopped rendering its chips or its picture would pass every assertion about its titles — 162 checks, needs the dev server |
| `node tools/notes_store_check.mjs` | the notes store against a stubbed Vercel Blob: seeding, the legacy read order, refusals that must not reseed, the rev check writing nothing on a conflict, the ten-minute and daily tiers on a synthetic clock, the ledger that replaced a list() per save — a steady save must cost exactly ONE advanced blob operation, and the upgrade from a ledger-less wrapper must neither duplicate nor lose a backup — assets by sha — 112 checks, no server needed |
| `node tools/work_check.mjs` | featured work, the work overlay, the code prompt, the games stack and the AI Lab, the four thumbnails keeping a height at four widths either side of the one-column breakpoint, and the tilde prompt — its mark, its one label, and that it opens OVER an overlay, is escaped back out of without taking that overlay with it, and still refuses while a field has the caret, its mark mirroring the X box for box and the SAME mark scaled to the Idea Vault’s own boxes — 130 checks, serves the repo itself |
| `node tools/sfx_check.mjs` | the sound library: the code opens it, the cards and their take counts come from the manifest on disk, a track with no file yet cannot be played, says so, and draws no waveform of nothing, NO transport anywhere and exactly one play button per card, a waveform whose bar count is the manifest's, a length read off the manifest before a byte is fetched, a real <audio> that plays, seeks from a REAL press 55% across that waveform (the harness serves Range, or Chrome refuses the seek), repeats on every card's button at once, takes a volume and stops from the header, previous/next walking the takes and wrapping with the picture following, a loop button HELD down looping until it comes up without latching repeat, folds remembered, 3/2/1 columns, search, a download that names the take and follows the dropdown, a take that SHIPS playing off disk as audio/ogg and being over a second later (the fixture is a 2s WAV; the library's takes are one-shots and the shortest is 0.13s), and closing the overlay leaving nothing audible — 79 checks, serves the repo itself |
| `node tools/music_check.mjs` | the music overlay: the four columns, the seeded repeat list, the three-state repeat, the centred transport, 20 size floors, the permanent bar, the cleared code boxes, the page that must not scroll, ` still working after a close, the found playing row, a Previous that never shuffles or leaves a track you are into, the code that starts the music, volume, the seek row, and docking (the iframe must not reload, the bar keeps one width, a playing track always has a control box, starting the Top Picks player closes the music feed, and the docked bar shows artwork rather than a live video), the five columns, the dead-track flag, the Top Picks bar being the same design and the same transport, and the arrow keys skipping tracks without taking them from anything else, the silent hold, the remote extension's relay and its two copies of the site list, and the vertical remote pill inside an overlay that covers the docked bar (in the host's top layer, hit-tested and pressed for real, every control read back off the BAR, gone again when the overlay closes or the music stops, folding to a circle that keeps its edge and its corner tab, and a corner it remembers while the fold it does not), and NO SOUND WITHOUT A CONTROL — an overlay opening over a playing music LIST must leave a bar behind, and a player forced into that state with no event at all must be found by the poll and put back on screen — 418 checks, serves the repo itself |
...
- **A keydown listener on your own subtree misses everything typed outside
  it.** The notes app bound its shortcuts to its root element; focus is
  regularly on the `<dialog>` or on `<body>`, neither of which is a
  descendant, so every shortcut died the moment a field blurred — and the
  Escape that followed reached the dialog and closed the whole overlay. Bind
  to `document` and guard on the subtree still being mounted.
- **Escape must be consumed by whatever it closed.** Without
  `stopPropagation` in each handler, one Escape closed the search AND the
  overlay behind it. Every rung of the ladder has to stop the key.
...
- **`scrollIntoView` on a smoothly-scrolling list returns the rect from BEFORE
  the scroll.** `.nt-rows` and `.sfx-list` both set `scroll-behavior: smooth`,
  so a coordinate measured in the same evaluate() as the scroll is the old one
  — the click then lands on whatever is there instead, which reads as a dead
  control. It cost two failures in one run: a seek that landed at 15% when it
  was driven at 55%, and a press-and-hold that never touched the button. Scroll
  with `behavior: 'instant'`, settle, then measure — and assert
  `elementFromPoint` is what you meant to hit.
