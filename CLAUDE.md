...
| `python tools/bake_sfx.py --check` | `assets/sfx/sfx.json` matches `sfx.txt`, and every file it names is there |
...
| `python tools/bake_sfx.py --cases` | the sound-manifest gate can still refuse — 15 cases, 13 of them refusals |
...
| `node tools/sfx_check.mjs` | the sound library: the code opens it, the cards and their take counts come from the manifest on disk, a track with no file yet cannot be played and says so, ONE transport moved between cards, a real <audio> that plays, seeks (the harness serves Range, or Chrome refuses the seek), repeats, takes a volume and stops, folds remembered, 3/2/1 columns, search, a download that names the take and follows the dropdown, an unsourced card that still offers one and carries no second label saying so, a take that SHIPS playing off disk as audio/ogg and being over a second later (the fixture is a 2s WAV; the library's takes are one-shots and the shortest is 0.13s), and closing the overlay leaving nothing audible — 56 checks, serves the repo itself |
