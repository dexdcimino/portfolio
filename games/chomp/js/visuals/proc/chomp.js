...
export function buildMawlingProc(stage, scene) {
  ...
  const bodyMat = unlitMat(scene, '#4DA89A');
  ...
  const hornMat = unlitMat(scene, '#2A2230', 0.12);
    ...
    const buildFurOn = (pivot, center, rx, ry, rz, count, excluded) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        for (let seg = 0; seg < 2; seg++) {
          ...
          const cc = (fur() < 0.25 ? BABYLON.Color3.FromHexString('#2E7D74') : baseBody).scale(shade);
  ...
  const eyeMat = unlitMat(scene, '#C8E84A', 1);
