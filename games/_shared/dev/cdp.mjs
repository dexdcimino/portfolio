...
//
// SHARED BY ALL FOUR GAMES. It began as games/surveyor/dev/cdp.mjs and moved
// here unchanged the day a second game needed a screenshot: nothing in it was
// ever about Surveyor, and three more copies of a Chrome launcher is three
// more places for the Downloads-folder note below to be forgotten. Surveyor's
// own dev/cdp.mjs is now a re-export, so its twenty harnesses import the same
// './cdp.mjs' they always did.
//
 ...
 */
export async function launch({ width = 900, height = 560, port = 0, gpu = false,
                              headed = false, app = null } = {}) {
  const exe = findChrome();
  const profile = mkdtempSync(join(tmpdir(), 'games-shots-'));
  ...
  /* WHICH PORT CHROME ACTUALLY TOOK.
     This used to default to 9222, which is fine for one harness and is a trap
     for two: six sessions work in this repo and every one of them launches
     through here. A second Chrome asked for a port already in use does not
     fail — it starts, loses the debugging socket, and the harness then talks to
     the FIRST browser's endpoint or to nothing at all. Both sessions hang with
     empty output and no error, which is the worst shape a failure can take. It
     cost two runs and half an hour on 2026-08-19 before anyone thought to look
     at the port.

     So the request is port 0 — Chrome binds a free one and writes it to
     DevToolsActivePort in the profile directory, which is a fresh mkdtemp per
     launch and therefore cannot be confused with anyone else's. Polled rather
     than slept on, because the file appears when the socket is listening and
     that is exactly the moment there is something to talk to.

     An explicit `port` still wins, for the one case that needs a known address:
     attaching a browser of your own to watch a harness run. */
  if (!port) {
    const stamp = join(profile, 'DevToolsActivePort');
    for (let i = 0; i < 200 && !port; i++) {
      if (existsSync(stamp)) {
        const first = readFileSync(stamp, 'utf8').split('\n')[0].trim();
...
async function connect(url) {
  ...
  return {
     ...
     * One command.
     *
     * `timeoutMs` is OPTIONAL and off by default, which keeps every existing
     * harness behaving exactly as it did — a perf run that evaluates a
     * sixty-second measurement must not be cut off by a number chosen here.
     *
     * It exists because a CDP command CAN hang forever, and two separate
     * gallery runs proved it the same way: the page navigated while a
     * `Runtime.evaluate` with awaitPromise was in flight, the promise it was
     * awaiting died with the old execution context, and no response was ever
     * sent. Not an error — silence. Chomp reloads itself on any keypress at the
     * death screen and the harness is always holding a key, so it hit this
     * every run; Surveyor hit it on a world swap. A harness that can hang
     * forever is a harness that stops a session dead, so anything driving a
     * page that might navigate under it should pass one.
     */
    send: (method, params = {}, timeoutMs = 0) => new Promise((resolve, reject) => {
      const n = ++id;
      let timer = null;
      const done = (fn) => (v) => { if (timer) clearTimeout(timer); fn(v); };
      pending.set(n, { resolve: done(resolve), reject: done(reject) });
      if (timeoutMs > 0) {
        timer = setTimeout(() => {
          pending.delete(n);
          reject(new Error(`${method} timed out after ${timeoutMs}ms `
            + '(the page most likely navigated out from under it)'));
        }, timeoutMs);
      }
 ...
 */
export async function evaluate(page, expression, timeoutMs = 0) {
  const r = await page.send('Runtime.evaluate', {
