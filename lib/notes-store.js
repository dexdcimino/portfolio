 ...
 *   notes/current.json            the live document: { rev, savedAt, doc }
 ...
 * the name this save would write. Returns what to write and what to prune.
 *
...
function backupPlan(names, stamp, everyMs, keep, now) {
  ...
  return { write, prune, kept: after.length - prune.length };
}
...
async function writeNotes(doc, baseRev, now = Date.now()) {
  ...
  const body = JSON.stringify({ rev, savedAt, doc });
  ...
  const daily = `${DAILY_DIR}${savedAt.slice(0, 10)}.json`;

  // COUNT WHAT IS THERE, then decide. The alternative -- deleting one per save
  // and trusting the count to stay put -- drifts the moment a save fails
  // halfway, and drifts silently: too many backups and too few both look like
  // a working system until someone needs a restore.
  ...
  if (plan.write) await store.write(stamp, body);

  const days = (await store.list(DAILY_DIR)).map((b) => b.pathname).filter((p) => p.endsWith('.json')).sort();
  const dayWrite = !days.includes(daily);
  if (dayWrite) await store.write(daily, body);
  const dayAll = dayWrite ? [...days, daily].sort() : days;
  const dayPrune = dayAll.slice(0, Math.max(0, dayAll.length - DAILY_KEEP));

  await store.write(CURRENT, body);

  if (plan.prune.length) await store.remove(plan.prune);
  if (dayPrune.length) await store.remove(dayPrune);

  return { rev, savedAt, backups: plan.kept, daily: dayAll.length - dayPrune.length };
}
...
module.exports = {
  ...
  readNotes, writeNotes, backupPlan, stampToMs, putAsset, getAsset,
};
