...
- Last tab in TOP PICKS, ten landscape cards laid out 5 across and 2 down. Each is one line of type (`Dog > Cat`) over its two vote controls, both centred in the card rather than split header-and-footer - the card is a small composition, not a layout. The free height is shared above the line, between the two and below the buttons, with the middle share weighted so the buttons read as sitting in the space rather than tucked under the text; the shares stay flexible because a fixed gap that looks right on one line overflows a label that wraps to two. No images anywhere in the panel; the carousel's decode gate already resolves instantly on an empty image set, so nothing waits on a decode that cannot happen.
...
- Each vote is a bordered rectangle button rather than an icon with a number beside it, so it reads as something to press: its own outline, fill and radius, all three taking the accent together when chosen. Fire sits left and poop right, ordered in the DOM rather than with CSS `order`, so the keyboard walks them the way they look.
...
- The card is a solid `#181d23` — the AI Lab rows' own fill, so the two families read as the same object at different sizes — at a 26px radius, rounder than the covers' 20.
